...
inputImage = cvtColor(inputImage, cv2.COLOR_BGR2Lab)
#cv2.imshow("lab_histeqeqLab", inputImage)
tmp1 = np.copy(inputImage)
print("LINE73 input looks like",inputImage[:, :, :])

tmp1[H1: H2+1, W1: W2+1, 0] = classhisteq(inputImage[H1: H2+1, W1: W2+1, 0])
tmp1 = cvtColor(tmp1, cv2.COLOR_Lab2BGR)
#tmp1[i, j] = cv2.equalizeHist(inputImage[i,j])
#cv2.imshow("lab_histeqeqW", tmp1)
